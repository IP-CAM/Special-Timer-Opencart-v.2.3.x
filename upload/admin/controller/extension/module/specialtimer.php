...
class ControllerExtensionModuleSpecialtimer extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/specialtimer', 'token=' . $this->session->data['token'], 'SSL')
		);


		$data['action'] = $this->url->link('extension/module/specialtimer', 'token=' . $this->session->data['token'], true);
       ...
       if (isset($this->request->post['special_time_config_search_product_page'])) {
		...
		$this->response->setOutput($this->load->view('extension/module/specialtimer.tpl', $data));
